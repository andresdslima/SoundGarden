    ...
    for (let i = 50; i < 53; i++) {
        divEventos.innerHTML += `
            <article class="evento card p-5 m-3">
                <h2 id="nomeData">${conteudoResposta[i].name} - ${conteudoResposta[i].scheduled}</h2>
                <h4 id="atracoes">${conteudoResposta[i].attractions}</h4>
                <p id="descricao">${conteudoResposta[i].description}</p>
                <a id="botao" data-id="${conteudoResposta[i]._id}" class="btn btn-primary open">Book tickets</a>
            </article>
        `;
    };
